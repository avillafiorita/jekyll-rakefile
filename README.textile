...
* URL substituions, to simplify deployment to subURIs
...
The mecahnism, which is inspired by Rails, is general and can be used to set specific sets of variables for previewing and deploying.  This is useful, for instance, when using "JAPR":https://github.com/kitsched/japr.  In fact, you might want to compile the assets only when deploying and leave them unbundled when debugging/previewing.  This can be achieved setting the "JAPR":https://github.com/kitsched/japr's @bundle@ variable, which needs to be set to @false@ when previewing and to @true@ when deploying.
